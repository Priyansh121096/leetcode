...
# Don't insort; rather sort at the end.
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
#         self.val = val
...
class Solution:
    ...
    def verticalTraversal(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        ans = []
        if self.minLevel < 0:
            ans += [[x[1] for x in sorted(nodes)] for nodes in self.nodes[self.minLevel:]]
        if self.maxLevel >= 0:
            ans += [[x[1] for x in sorted(nodes)] for nodes in self.nodes[:self.maxLevel+1]]
